 ...
 */
export function Boolean(__model__: Sim, a_entities: string|string[], b_entities: string|string[], method: _EBooleanMethod): string[] {
    ...
    if (b_pgons_i.length === 0) {
        switch (method) {
            ...
            case _EBooleanMethod.SYMMETRIC:
                ...
                return idsMake(__model__.modeldata.funcs_common.copyGeom(a_ents_arr, false)) as string[];
            default:
                return [];
        }
    }
    ...
    const new_pgons: string[] = idsMakeFromIdxs(ENT_TYPE.PGON, new_pgons_i) as string[];
    // const new_pgons: string[] = idsMake(new_pgons_i.map( pgon_i => [ENT_TYPE.PGON, pgon_i] as string )) as string[];
    for (const new_pgon of new_pgons) {
        result_ents.push(new_pgon);
    }
    const new_plines: string[] = idsMakeFromIdxs(ENT_TYPE.PLINE, new_plines_i) as string[];
    // const new_plines: string[] = idsMake(new_plines_i.map( pline_i => [ENT_TYPE.PLINE, pline_i] as string )) as string[];
...
function _booleanPgons(__model__: Sim, pgons_i: number|number[], b_shape: Shape,
...
function _booleanPlines(__model__: Sim, plines_i: number|number[], b_shape: Shape,
        method: _EBooleanMethod, posis_map: TPosisMap): number[] {
