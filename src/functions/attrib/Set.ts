import { getArrDepth, Sim, idsBreak, TAttribDataTypes, string, string } from '../../mobius_sim';
...
export function Set(
    ...
    if (this.debug) {
        ...
        // }
        ents_arr = checkIDs(__model__, fn_name, "entities", entities, [ID.isNull, ID.isID, ID.isIDL1], null) as string | string[];
...
function _setModelAttrib(__model__: Sim, attrib_name: string, attrib_value: TAttribDataTypes, idx_or_key?: number | string): void {
...
function _getEntsIndices(__model__: Sim, ents_arr: string[]): number[] {
    const ent_type: number = ents_arr[0][0];
    const ents_i: number[] = [];
    for (let i = 0; i < ents_arr.length; i++) {
