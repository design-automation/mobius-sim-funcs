...
export function _rayOrisDirsTjs(__model__: Sim, origins: Txyz[] | TRay[] | TPlane[], offset: number): [THREE.Vector3, THREE.Vector3][] {
...
export function _getUniquePosis(__model__: Sim, ents_arr: string[]): number[] {
...
export function _cytoscapeGetElements(
    ...
    if (__model__.modeldata.attribs.query.hasEntAttrib(ENT_TYPE.EDGE, "weight")) {
        has_weight_attrib = __model__.modeldata.attribs.query.getAttribDataType(ENT_TYPE.EDGE, "weight") === EAttribDataTypeStrs.NUMBER;
    }
    // edges, starts empty
    ...
    if (directed) {
        ...
        for (const edge_i of Array.from(set_edges_i)) {
            const edge_posis_i: number[] = __model__.modeldata.geom.nav.navAnyToPosi(ENT_TYPE.EDGE, edge_i);
            let weight = 1.0;
            if (has_weight_attrib) {
                weight = __model__.modeldata.attribs.get.getEntAttribVal(ENT_TYPE.EDGE, edge_i, "weight") as number;
            } else {
        ...
        for (const edge_i of Array.from(set_edges_i)) {
            let edge_posis_i: number[] = __model__.modeldata.geom.nav.navAnyToPosi(ENT_TYPE.EDGE, edge_i);
