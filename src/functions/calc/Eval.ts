...
function _eval(__model__: Sim, ents_arr: string|string[], t_param: number): Txyz|Txyz[] {
    if (getArrDepth(ents_arr) === 1) {
        ...
        if (ent_type === ENT_TYPE.EDGE || ent_type === ENT_TYPE.WIRE || ent_type === ENT_TYPE.PLINE) {
            ...
            const wires_arrs: string[] = wires_i.map(wire_i => [ENT_TYPE.WIRE, wire_i] as [ENT_TYPE, number]);
            return wires_arrs.map( wires_arr => _eval(__model__, wires_arr, t_param) ) as Txyz[];
        }
    } else {
        return (ents_arr as string[]).map( ent_arr => _eval(__model__, ent_arr, t_param) ) as Txyz[];
    }
}
