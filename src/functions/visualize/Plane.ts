...
function _visPlane(__model__: Sim, planes: TPlane|TPlane[], scale: number): string[] {
    if (getArrDepth(planes) === 2) {
        ...
        return [
            [ENT_TYPE.PLINE, x_pline_i],
            [ENT_TYPE.PLINE, y_pline_i],
            [ENT_TYPE.PLINE, z_pline_i],
            [ENT_TYPE.PLINE, plane_i]
        ];
    } else {
        const ents_arr: string[] = [];
        for (const plane of planes) {
            const plane_ents: string[] = _visPlane(__model__, plane as TPlane, scale);
