 ...
 */
export async function ModelCompare(__model__: Sim, input_data: string): Promise<string> {
    const input_data_str: string = await _getFile(input_data);
    if (!input_data_str) {
        throw new Error('Invalid imported model data');
    }
    const input_model = new Sim();
...
export function _Async_Param_ModelCompare(__model__: Sim, input_data: string): Promise<string> {
    return null;
}
