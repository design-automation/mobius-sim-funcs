...
function _lineMaterial(__model__: Sim, ents_arr: string[], material: string): void {
    if (!__model__.modeldata.attribs.query.hasEntAttrib(ENT_TYPE.PLINE, EAttribNames.MATERIAL)) {
        __model__.modeldata.attribs.add.addAttrib(ENT_TYPE.PLINE, EAttribNames.MATERIAL, EAttribDataTypeStrs.STRING);
    }
    for (const ent_arr of ents_arr) {
        ...
        for (const pline_i of plines_i) {
            __model__.modeldata.attribs.set.setEntAttribVal(ENT_TYPE.PLINE, pline_i, EAttribNames.MATERIAL, material);
        }
    }
}
function _meshMaterial(__model__: Sim, ents_arr: string[], material: string[]): void {
    if (!__model__.modeldata.attribs.query.hasEntAttrib(ENT_TYPE.PGON, EAttribNames.MATERIAL)) {
        __model__.modeldata.attribs.add.addAttrib(ENT_TYPE.PGON, EAttribNames.MATERIAL, EAttribDataTypeStrs.LIST);
    }
    for (const ent_arr of ents_arr) {
        const [ent_type, ent_i]: [number, number] = ent_arr as string;
        const pgons_i: number[] = __model__.modeldata.geom.nav.navAnyToPgon(ent_type, ent_i);
        for (const pgon_i of pgons_i) {
            __model__.modeldata.attribs.set.setEntAttribVal(ENT_TYPE.PGON, pgon_i, EAttribNames.MATERIAL, material);
        }
    }
}
