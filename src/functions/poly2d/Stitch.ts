 ...
 */
export function Stitch(__model__: Sim, entities: string | string[], tolerance: number): string[] {
    ...
    //     ents_arr2.push([ENT_TYPE.PLINE, pline_i]);
    //     const ent_edges_i: number[] = __model__.modeldata.geom.nav.navAnyToEdge(ENT_TYPE.PLINE, pline_i);
    ...
    for (const [ent_type, ent_i] of new_ents_arr) {
        ...
        for (const ent_wire_i of ent_wires_i) {
            const wire_edges_i: number[] = __model__.modeldata.geom.nav.navAnyToEdge(ENT_TYPE.WIRE, ent_wire_i);
    ...
    for (const edge_i of map_edge_i_to_isects.keys()) {
        ...
        if (new_sta) {
            ...
            const pline_i: number = __model__.modeldata.geom.nav.navAnyToPline(ENT_TYPE.EDGE, edge_i)[0];
...
function _stitchGetEdgeData(__model__: Sim, edge_i: number, tol: [number, number],
