 ...
 * key]`.
 * @param operator_enum Enum, the operator to use for filtering: `'==', '!=', '>=', '<=', '>', '<'` or `'='`.
 ...
 */
export function Filter(__model__: Sim,
    ...
    // do the query
    const found_ents_arr: string[]|string[][] = _filter(__model__, ents_arr, attrib_name, attrib_idx_key, op_type, value);
    // return the result
...
function _filter(__model__: Sim, ents_arr: string[]|string[][],
        name: string, idx_or_key: number|string, op_type: EFilterOperatorTypes, value: TAttribDataTypes): string[]|string[][] {
    ...
    if (depth === 2) {
        ...
        return query_result.map( entity_i => [ent_type, entity_i]) as string[];
    } else { // depth === 3
        // TODO Why do we want this option?
        // TODO I cannot see any reason to return anything buy a flat list
        ents_arr = ents_arr as string[][];
        return ents_arr.map(ents_arr_item => _filter(__model__, ents_arr_item, name, idx_or_key, op_type, value)) as string[][];
    }
}
