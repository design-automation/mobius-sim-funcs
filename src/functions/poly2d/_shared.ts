import { ENT_TYPE, Sim, string, Txyz } from '../../mobius_sim';
...
export function _getPgons(__model__: Sim, ents_arr: string[]): number[] {
...
export function _getPgonsPlines(__model__: Sim, ents_arr: string[]): [number[], number[]] {
    const set_pgons_i: Set<number> = new Set();
...
export function _getPosis(__model__: Sim, ents_arr: string[]): number[] {
...
export function _getPosiFromMap(__model__: Sim, x: number, y: number, posis_map: TPosisMap): number {
...
// mobius -> clipperjs
export function _convertPgonToShape(__model__: Sim, pgon_i: number, posis_map: TPosisMap): Shape {
    const wires_i: number[] = __model__.modeldata.geom.nav.navAnyToWire(ENT_TYPE.PGON, pgon_i);
...
// clipperjs
export function _convertPgonsToShapeUnion(__model__: Sim, pgons_i: number[], posis_map: TPosisMap): Shape {
...
// clipperjs
function _convertPgonsToShapeJoin(__model__: Sim, pgons_i: number[], posis_map: TPosisMap): Shape {
    let result_shape: Shape = null;
    for (const pgon_i of pgons_i) {
        const shape: Shape = _convertPgonToShape(__model__, pgon_i, posis_map);
        if (result_shape == null) {
...
// mobius -> clipperjs
export function _convertWireToShape(__model__: Sim, wire_i: number, is_closed: boolean, posis_map: TPosisMap): Shape {
    const shape_coords: TClipPaths = [];
    shape_coords.push([]);
    const posis_i: number[] = __model__.modeldata.geom.nav.navAnyToPosi(ENT_TYPE.WIRE, wire_i);
    for (const posi_i of posis_i) {
...
// mobius -> clipperjs
export function _convertPlineToShape(__model__: Sim, pline_i: number, posis_map: TPosisMap): Shape {
    ...
    const shape_coords: TClipPaths = [];
    shape_coords.push([]);
    const posis_i: number[] = __model__.modeldata.geom.nav.navAnyToPosi(ENT_TYPE.PLINE, pline_i);
    for (const posi_i of posis_i) {
        const xyz: Txyz = __model__.modeldata.attribs.posis.getPosiCoords(posi_i);
        const coord: IClipCoord = { X: xyz[0], Y: xyz[1] };
        shape_coords[0].push(coord);
...
// clipperjs -> mobius
export function _convertShapesToPgons(__model__: Sim, shapes: Shape | Shape[], posis_map: TPosisMap): number[] {
...
// clipperjs
export function _convertShapeToPlines(__model__: Sim, shape: Shape, is_closed: boolean, posis_map: TPosisMap): number[] {
    shape.scaleDown(SCALE);
    const sep_shapes: Shape[] = shape.separateShapes();
...
// clipperjs
export function _convertShapeToCutPlines(__model__: Sim, shape: Shape, posis_map: TPosisMap): number[] {
    shape.scaleDown(SCALE);
    const sep_shapes: Shape[] = shape.separateShapes();
...
export function _convexHull(__model__: Sim, posis_i: number[]): number[] {
...
export function _offsetPgon(__model__: Sim, pgon_i: number, dist: number,
...
export function _offsetPline(__model__: Sim, pline_i: number, dist: number,
    options: IClipOffsetOptions, posis_map: TPosisMap): number[] {
    const wire_i: number = __model__.modeldata.geom.nav.navPlineToWire(pline_i);
    const is_closed: boolean = __model__.modeldata.geom.query.isWireClosed(wire_i);
    if (is_closed) {
