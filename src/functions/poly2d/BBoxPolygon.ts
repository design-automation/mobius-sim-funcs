...
export function BBoxPolygon(__model__: Sim, entities: string|string[], method: _EBBoxMethod): string {
    ...
    return idMake(ENT_TYPE.PGON, pgon_i) as string;
}
function _bboxAABB(__model__: Sim, posis_i: number[]): number {
...
function _bboxOBB(__model__: Sim, posis_i: number[]): number {
    // posis
