...
export function ShortestPath(
    ...
    if (return_dists) {
        dict.source_posis = idsMakeFromIdxs(ENT_TYPE.POSI, source_posis_i) as string[];
        dict.distances = source_posis_i.length === 1 ? all_path_dists[0] : all_path_dists;
    }
    if (return_counts) {
        dict.edges = idsMakeFromIdxs(ENT_TYPE.EDGE, Array.from(map_edges_i.keys())) as string[];
        dict.edges_count = Array.from(map_edges_i.values());
        dict.posis = idsMakeFromIdxs(ENT_TYPE.POSI, Array.from(map_posis_i.keys())) as string[];
        dict.posis_count = Array.from(map_posis_i.values());
    }
    if (return_paths) {
        dict.edge_paths = idsMakeFromIdxs(ENT_TYPE.EDGE, edge_paths) as string[][];
        dict.posi_paths = idsMakeFromIdxs(ENT_TYPE.POSI, posi_paths) as string[][];
    }
    return dict;
}
