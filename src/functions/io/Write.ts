import { download, Sim } from '../../mobius_sim';
 ...
 */
export async function Write(__model__: Sim, data: string, file_name: string, data_target: _EIODataTarget): Promise<Boolean> {
    try {
...
export function _Async_Param_Write(__model__: Sim, data: string, file_name: string, data_target: _EIODataTarget): Promise<Boolean> {
    return null;
}
